...
int main (int argc, char *argv[]) {
  ...
  int fwrite = 0;
  int status=0;
  float mytsamp = NNATINTS*NCORRINTS*4*8.192e-6;
  int NINTS;
  ...
  while (!observation_complete) {
    ...
    data = (float *)block; // order is [136 baseline, 1536 freq, 2 pol, 2 ri]

    for (int i=0;i<36864;i++) summed_vis[i] = block[i];
    
    syslog(LOG_INFO,"read block");
	...
	char *ttype[] = {"VIS"};
	char *tform[] = {"36864E"}; // assumes classic npts
	char *tunit[] = {"\0"};
	char *wsrcnam = srcnam;
	...
	fwrite=1;
      ...
      fits_write_col(fptr, TFLOAT, 1, rownum, 1, 36864, summed_vis, &status);
      rownum += 1;
