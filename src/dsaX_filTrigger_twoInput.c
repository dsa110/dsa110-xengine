...
void dsaX_dbgpu_cleanup (dada_hdu_t * in, dada_hdu_t * in2);
...
void dsaX_dbgpu_cleanup (dada_hdu_t * in, dada_hdu_t * in2)
{
  ...
  if (dada_hdu_unlock_read (in2) < 0)
    {
      syslog(LOG_ERR, "could not unlock read on hdu_in2");
    }
...
int main (int argc, char *argv[]) {
  ...
  while ((arg=getopt(argc,argv,"i:c:j:k:db:n:hz")) != -1)
    {
      switch (arg)
	{
	case 'i':
  ...
  if (!header_in)
    {
      ...
      dsaX_dbgpu_cleanup (hdu_in, hdu_in2);
      return EXIT_FAILURE;
    }
  ...
  while (!observation_complete) {
    ...
    in_data2 = ipcio_open_block_read (hdu_in2->data_block, &bytes_read, &block_id);
	...
	for (int i=0;i<NBEAMS_PER_BLOCK/2;i++) {
	...
	for (int i=0;i<NBEAMS_PER_BLOCK/2;i++) {
	  ...
	  start_byte = i*NSAMPS_PER_BLOCK*NCHAN_FIL + (specnum-current_specnum)*NCHAN_FIL;
	  memcpy(extData + (i+NBEAMS_PER_BLOCK/2)*NSAMPS_PER_BLOCK*NCHAN_FIL, in_data2 + start_byte, bytes_to_copy);
	...
	for (int i=0;i<NBEAMS_PER_BLOCK/2;i++) {
	  ...
	  start_byte = i*NSAMPS_PER_BLOCK*NCHAN_FIL;
	  memcpy(extData + (i+NBEAMS_PER_BLOCK/2)*NSAMPS_PER_BLOCK*NCHAN_FIL + bytes_copied, in_data2 + start_byte, bytes_to_copy);
