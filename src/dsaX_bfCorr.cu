...
void initialize(dmem * d, int bf) {
  ...
  if (bf==1) {
    ...
    cudaMalloc((void **)(&d->d_bigbeam_r), sizeof(half)*(NPACKETS_PER_BLOCK/8)*(NCHAN_PER_PACKET/8)*(NBEAMS/2));
    cudaMalloc((void **)(&d->d_bigbeam_i), sizeof(half)*(NPACKETS_PER_BLOCK/8)*(NCHAN_PER_PACKET/8)*(NBEAMS/2));
...
void dbeamformer(dmem * d) {
  ...
  cublasOperation_t transa = CUBLAS_OP_T;
  cublasOperation_t transb = CUBLAS_OP_N;
  const int m = NPACKETS_PER_BLOCK/8;
  ...
  const long long int strideC = (NPACKETS_PER_BLOCK/8)*NBEAMS/2;
  ...
  for (int iArm=0;iArm<2;iArm++) {
    ...
    // zero out output arrays
    cudaMemset(d->d_bigbeam_r,0,(NPACKETS_PER_BLOCK/8)*(NCHAN_PER_PACKET/8)*(NBEAMS/2)*sizeof(half));
    cudaMemset(d->d_bigbeam_i,0,(NPACKETS_PER_BLOCK/8)*(NCHAN_PER_PACKET/8)*(NBEAMS/2)*sizeof(half));
    cudaDeviceSynchronize();
    ...
    begin = clock();
    dim3 dimBlock(16, 8), dimGrid((NBEAMS/2)*(NPACKETS_PER_BLOCK/8)/16, (NCHAN_PER_PACKET/8)/16);
    transpose_scale_bf<<<dimGrid,dimBlock>>>(d->d_bigbeam_r,d->d_bigbeam_i,d->d_bigpower+iArm*(NPACKETS_PER_BLOCK/8)*(NCHAN_PER_PACKET/8)*(NBEAMS/2));
    end = clock();
    d->outp += (float)(end - begin) / CLOCKS_PER_SEC;
