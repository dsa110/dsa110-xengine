...
__global__ void power_sum_and_transpose_output(half * dra, half * drb, half * dia, half * dib, half * outp) {
...
__global__ void sum_transpose_and_scale_output(half * outp, unsigned char * odata) {
...
void dbeamformer(dmem * d) {
  ...
  for (int iArm=0;iArm<2;iArm++) {
    ...
    power_sum_and_transpose_output<<<dimGrid2,dimBlock2>>>(d->d_bigbeam_a_r,d->d_bigbeam_b_r,d->d_bigbeam_a_i,d->d_bigbeam_b_i,d->d_htx);

    dim3 dimBlock(32, 8), dimGrid((NBEAMS/2)/32,(NPACKETS_PER_BLOCK/4)*(NCHAN_PER_PACKET/8)/32);
    sum_transpose_and_scale_output<<<dimGrid,dimBlock>>>(d->d_htx,d->d_bigpower+iArm*(NPACKETS_PER_BLOCK/4)*(NCHAN_PER_PACKET/8)*(NBEAMS/2));
